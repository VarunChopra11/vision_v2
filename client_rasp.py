    ...
    try:
        async with websockets.connect(uri, max_size=None, ping_interval=None) as ws:
                ...
                try:
                    while True:
                        if is_playing_audio.is_set():
                            ...
                            continue
                        data = await run_in_thread(stream_send.read, CHUNK, exception_on_overflow=False)
            ...
            async def send_video():
                try:
                    while True:
                        frame = await run_in_thread(picam2.capture_array)
                        
                        # Optional enhancement: apply gamma correction
                ...
                try:
                    while True:
                        ...
                        if msg_type == "audio_from_gemini":
                            ...
                            audio_data_bytes = base64.b64decode(msg_json["data"])
                            sample_rate = msg_json.get("sample_rate", 24000)

                            if stream_play is None:
                                stream_play = audio.open(format=pyaudio.paInt16, channels=CHANNELS, rate=sample_rate, output=True)
            ...
            await asyncio.gather(send_audio(), send_video(), receive_messages(), send_video_to_show())
